...
class CircleChanger(object):
    ...
    def get_distance_from(self, point):
        ...
        length = center.x - point.x
        height = center.y - point.y
        distance = math.sqrt((length ** 2) + (height ** 2))
    ...
    def swallow(self, other_circle_changer):
        ...
        Type hints:
            ...
            :rtype CircleChanger
        """
        length_n = self.circle.center.x + other_circle_changer.circle.center.x
        height_n = self.circle.center.y  + other_circle_changer.circle.center.y
        new_center = rg.Point(length_n / 2, height_n / 2)
        radius = (self.get_distance_from(other_circle_changer.circle.center)) / 2
        fill_color = 'red'
        colors = self.colors + other_circle_changer.colors
        circle3 = CircleChanger(new_center.x, new_center.y,radius,fill_color,colors)
        return circle3
